...
df1i['grade']=df2i['grade']
df_final=pd.concat([df1i, df3i], axis=0)
df_final
```
...
```{code-cell} ipython3
m1.rename(columns={'prenom': 'first_name', 'nom': 'last_name'}, inplace=True)
m3.rename(columns={'first': 'first_name', 'last': 'last_name'}, inplace=True)
m12 = pd.merge(m1, m2, left_on=['first_name', 'last_name'], right_on=['first_name', 'last_name'], how='outer')
df_final = pd.concat([m12, m3], ignore_index=True)
df_final
```
